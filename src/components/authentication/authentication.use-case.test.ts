...
describe("Authenticate use case", () => {
  ...
  test("Should be able to authenticate", async () => {
    const response = await authenticationUseCase.execute({email: user.email, password: user.password})
    expect(response.token).toBeDefined()
  })

  test("Should not be able to authenticate when invalid password", async () => {
    await expect(authenticationUseCase.execute({email: user.email, password: "randompassword"})).rejects.toThrow("Invalid email or password")
  })

  test("Should not be able to authenticate when invalid email", async () => {
    await expect(authenticationUseCase.execute({email: "emailnotexists@gmail.com", password: user.password})).rejects.toThrow("Invalid email or password")
  })
})