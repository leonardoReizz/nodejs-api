...
describe("Get user controller", () => {
  ...
  beforeAll(async () => {
    ...
    await authenticationUseCase.execute({email: user.email, password: user.password}).then((res) => token = `Bearer ${res.token}`)
  })
  ...
  test("Should not be able to get user when invalid token", async () => {
    const res = await request(app).get("/user").set("Authorization", `${token}F0s`)
